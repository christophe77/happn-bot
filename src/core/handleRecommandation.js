...
async function handleRecommandation(user, savePictures, criterias) {
    if (criterias) {
        try {
            const renewableLikes = await handleWithCriterias(user, savePictures, criterias);
            return renewableLikes || 0;
        } catch (error) {
            console.log(colors.bgRed, `handleWithCriterias failed ${error.message}`, colors.reset);
        }
    } else {
        try {
            ...
            console.log(colors.bgRed, `handleWithoutCriterias failed ${error.message}`, colors.reset);
        }
    }
}
